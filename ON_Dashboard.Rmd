...
```{r}
casestatus %>% 
  dplyr::select(Reported.Date, Total_Lineage_B.1.1.7,Total_Lineage_B.1.351,Total_Lineage_P.1) %>% 
  dplyr::mutate(Total_Lineage_B.1.1.7 = coalesce(Total_Lineage_B.1.1.7, 0), 
                Total_Lineage_B.1.351 = coalesce(Total_Lineage_B.1.351, 0),
                Total_Lineage_P.1 = coalesce(Total_Lineage_P.1, 0)) %>% 
  dplyr::filter(Total_Lineage_B.1.1.7 > 0) %>% 
  
  plot_ly(x = ~Reported.Date, 
          y = ~Total_Lineage_B.1.1.7, 
          type = 'bar', name = 'UK', 
          marker = list(color = '#6EC5E9')) %>% 
  
  add_trace(y = ~Total_Lineage_B.1.351, 
            name = 'South African ', 
            marker = list(color = '#FFC56C')) %>% 
  
  add_trace(y = ~Total_Lineage_P.1, 
            name = 'Brazilian ', 
            marker = list(color = '#FF5959')) %>% 
...
Vaccine_data %>% 
  dplyr::filter(!is.na(previous_day_doses_administered)) %>% 
  ...
  plot_ly(x = ~report_date, 
        ...
        text = ~paste(report_date, "<br>",
                      "Administered:", format(previous_day_doses_administered, big.mark = ","),  sep = ""), 
        hoverinfo = "text") %>% 
  
  layout(title = "Previous Day - Doses Administered",
         xaxis = list(type = 'date', 
                      tickformat = "%B <br> %Y",
                      title = '', 
                      rangeselector = list(buttons = buttons_time)),
...
Vaccine_data %>% 
  ...
  layout(title = "Vaccination Progress",
         xaxis = list(title = ""), 
...
casestatus %>%   
  ...
  plot_ly(x = ~Month_Yr, y = ~ICU, type = 'bar', name = 'ICU', 
          marker = list(color = '#FFC56C')) %>% 
  add_trace(x = ~Month_Yr, y = ~Total_Hospitalized,name = 'Hospitalizations', 
            marker = list(color = '#dcdcdc')) %>% 
