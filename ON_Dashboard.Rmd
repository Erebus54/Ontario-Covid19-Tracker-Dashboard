...
casestatus %>%
  ...
  dplyr::mutate(Resolved = ifelse(is.na(Resolved), 0, Resolved),
    ...
    TotalCases_ChangePct = round(TotalCases_Change/Total.Cases*100 ,1), 
    Resolved_ChangePct = round(Resolved_Change/Resolved*100 ,1),
    Deaths_ChangePct = round(Deaths_Change/Deaths*100 ,1), 
    
    # Relative change computations produce NaN values
    # replace with 0.00 values 
    TotalCases_ChangePct = ifelse(is.na(TotalCases_ChangePct), 0.00, TotalCases_ChangePct), 
    Resolved_ChangePct = ifelse(is.na(Resolved_ChangePct), 0.00, Resolved_ChangePct), 
    Deaths_ChangePct = ifelse(is.na(Deaths_ChangePct), 0.00, Deaths_ChangePct)) %>% 
  
plot_ly(x = ~Reported.Date, y = ~TotalCases_ChangePct, 
        type = 'bar', 
        name = 'Cases', 
        marker = list(color = '#FFC56C')) %>% 
  
  add_trace(y = ~Resolved_ChangePct, 
            name = 'Resolved', 
            marker = list(color = '#6EC5E9')) %>% 
  
  add_trace(y = ~Deaths_ChangePct, 
            name = 'Deaths', 
            marker = list(color = '#FF5959')) %>% 
  ...
  layout(title = "", 
         ...
         hovermode = "x-unified", 
         paper_bgcolor = 'transparent', 
         legend=list(title=list(text='<b> Cases </b>'))) 
```
...
plot_ly(x = ~Reported.Date, y = ~TotalCasesPerCapita, 
  ...
  layout(title = "", 
         ...
         hovermode = "x-unified", 
         paper_bgcolor = 'transparent', 
         legend=list(title=list(text='<b> Per Capita Rates </b>'))) 
```
...
plot_ly(data = casestatus,
        ...
        text = ~paste(Reported.Date, "<br>", 
                      format(Confirmed.Positive, big.mark = ",")
                      , 'Tested Positive', sep = " "), 
        hoverinfo = 'text') %>% 
...
casestatus %>%   
  ...
  plot_ly(x = ~Reported.Date, 
          ...
          marker = list(color = '#F8F8F8', line = list(width = 0.5,
                                                       color = 'black')), 
          text = ~paste(format(Reported.Date, "%A, %B %d, %Y"), "<br>",
                        'Positivity Rate: ', Positive_pct, '%', sep = ""), 
          hoverinfo = 'text',
          
          #filters for values larger than 0 in Y column 
          transforms = list(list(type = 'filter',
                                 target = 'y', 
                                 operation = '>', 
                                 value = 0.0))) %>% 
  
  add_trace(name = "Negative", 
            x = ~Reported.Date,
            y = ~Negative_pct, 
            marker = list(color = '#808080'), 
            text = ~paste(format(Reported.Date, "%A, %B %d, %Y"), "<br>",
                          'Negativity Rate: ', Positive_pct, '%', sep = ""), 
            hoverinfo = 'text') %>% 
  ...
  layout(title = "",
         ...
         legend=list(title=list(text='<b> Test Outcome: </b>')))
```
...
casestatus %>%   
  dplyr::select(Reported.Date, Confirmed.Positive, Total.tests.completed.in.the.last.day) %>% 
  dplyr::filter(!is.na(Total.tests.completed.in.the.last.day)) %>% 
  dplyr::mutate(TestPositiveRate = round((Confirmed.Positive/Total.tests.completed.in.the.last.day)*100,1), 
                # calcualting the tests per capita 
                TestsPerCapitaM = round((Total.tests.completed.in.the.last.day/14750000)*1000000,0)) %>% 
...
plot_ly(x = ~Reported.Date, 
        ...
        marker = list(color = '#FFD300'),
        text = ~paste(format(Reported.Date, "%A, %B %d, %Y"), "<br>",
                      format(TestsPerCapitaM, big.mark = ","), " Tests per Capita (1M)", sep = ""), 
        hoverinfo = "text")%>% 
...
Vaccine_data <- read.csv("./datasets/COVID-19 Vaccine Data.csv") %>% 
  dplyr::mutate(report_date = as.Date(report_date, format = "%Y-%m-%d"), 
                total_doses_in_fully_vaccinated_individuals = as.numeric(gsub(",", "", total_doses_in_fully_vaccinated_individuals)),
                total_individuals_fully_vaccinated = as.numeric(gsub(",", "", total_individuals_fully_vaccinated))) 
...
Vaccine_data %>% 
  ...
  plot_ly(x = ~report_date, 
          ...
          type = 'scatter',
          mode = "lines", 
          line = list(color = "#405DE6", width = 4), 
          text = ~paste(format(report_date, "%A, %B %d, %Y"), "<br>",
                        format(total_individuals_fully_vaccinated, big.mark = ","), " total individuals fully vaccinated", sep = ""), 
          hoverinfo = "text") %>% 
  
  layout(title = "",
         yaxis = list(title = 'Vaccinations Completed'),
         xaxis = list(type = 'date', 
                      tickformat = "%B <br> %Y",
                      title = '', 
                      rangeselector = list(buttons = buttons_time)),
         hovermode = "x-unified", 
         hoverlabel=list(bgcolor="#405DE6"),
         plot_bgcolor  = "black",
         paper_bgcolor='black', 
         font = list(color = '#C0C0C0'))
```

### <b> Daily Vaccinations </b>

```{r, context="render", echo=FALSE}
Vaccine_data %>% 
  dplyr::select(report_date, total_individuals_fully_vaccinated) %>% 
  # NA values before this date, we filter out 
  dplyr::filter(report_date >= "2021-01-06") %>% 
  dplyr::mutate(daily_vaccinations = total_individuals_fully_vaccinated - lag(total_individuals_fully_vaccinated, 1, default = 0)) %>% 
  
  
  plot_ly(x = ~report_date, 
          y = ~daily_vaccinations, 
          type = 'bar',
          marker = list(color = "#FF00BF"), 
          text = ~paste(format(report_date, "%A, %B %d, %Y"), "<br>",
                        "New Vaccinations: ", format(daily_vaccinations, big.mark = ","),sep = ""), 
          hoverinfo = "text") %>% 
  ...
  layout(title = "",
         ...
         xaxis = list(type = 'date', 
                      ...
                      rangeselector = list(buttons = buttons_time)),
         hovermode = "x-unified", 
         hoverlabel=list(bgcolor="#FF00BF"),
         plot_bgcolor  = "black",
         paper_bgcolor='black', 
         font = list(color = '#C0C0C0'))
```
...
Vaccine_data <- Vaccine_data %>% 
  dplyr::mutate(report_date = as.Date(report_date, format = "%m/%d/%Y"),
                Pct = round(total_individuals_fully_vaccinated/14.57e6*100, 2))
...
Vaccine_data %>% 
  ...
  plot_ly(x = ~report_date, 
          ...
          type = 'scatter',
          mode = "lines+markers", 
          line = list(color = "#6d2aff", width = 4), 
          marker = list(color = "#9b6dff", size = 8), 
          text = ~paste(format(report_date, "%A, %B %d, %Y"), "<br>",
                        format(Pct, big.mark = ","), " % Vaccinated", sep = ""), 
          hoverinfo = "text") %>% 
  
  layout(title = "",
         yaxis = list(title = 'Vaccinations (% of Ontario Population)'),
         xaxis = list(type = 'date', 
                      tickformat = "%B <br> %Y",
                      title = '', 
                      rangeselector = list(buttons = buttons_time)),
         hovermode = "x-unified", 
         hoverlabel=list(bgcolor="#6d2aff"),
         plot_bgcolor  = "black",
         paper_bgcolor='black', 
         font = list(color = '#C0C0C0'))
```
...
plot_ly(casestatus, x = ~Reported.Date, y = ~Number.of.patients.hospitalized.with.COVID.19, 
        ...
        #marker = list(color = '#2C82C9', size = 3), 
        line = list(color = '#6EC5E9', width = 2), 
        text = ~paste(Reported.Date, "<br>",
                      format(Number.of.patients.hospitalized.with.COVID.19, big.mark = ","), " Total Hospitalized", sep = ""), 
        hoverinfo = "text") %>%
  
  add_trace(y = ~Number.of.patients.in.ICU.with.COVID.19, 
            name = 'ICU', 
            #marker = list(color = '#FCB941', size = 3),
            line = list(color = '#FFD300', width = 2), 
            text = ~paste(Reported.Date, "<br>",
                          Number.of.patients.in.ICU.with.COVID.19, " ICU Patients", sep = ""), 
            hoverinfo = "text") %>% 
  
  add_trace(y = ~Number.of.patients.in.ICU.on.a.ventilator.with.COVID.19, 
            name = 'Ventilator', 
            #marker = list(color = '#FC6042', size = 3),
            line = list(color = '#FF1C51', width = 2), 
            text = ~paste(Reported.Date, "<br>",
                          Number.of.patients.in.ICU.on.a.ventilator.with.COVID.19, " Vent. Patients", sep = ""), 
            hoverinfo = "text") %>% 
...
ongoingOutbreaks %>% 
  ...
  layout(barmode = 'stack',
         ...
         xaxis = list(type = 'date', 
                      ...
                      title = '', 
                      rangeselector = list(buttons = buttons_time)),
         yaxis = list(title = "Frequency of Ongoing Outbreaks (%)", 
                      dtick = 25, 
                      tick0 = 0, 
                      tickmode = "linear"), 
         legend=list(title=list(text='<b> Workplace Groups: </b>'))
...
plot_ly(data = reg_Breakdown, 
  ...
  layout(title = "Outcome Composition",
         xaxis = list(title = ""), 
         yaxis = list(title = "Outcome Frequency (%)"), 
         barmode = 'stack',
         hovermode = "y-unified", 
         hoverlabel=list(bgcolor="black"),
